...
const CandidateSidebar = ({ mobileOpen, setMobileOpen }) => {
  const location = useLocation();
  const navigate = useNavigate();
  const { id } = useParams();

  const buildPath = (subpath) =>
    subpath === '/' ? `/candidate/${id}` : `/candidate/${id}${subpath}`;

  const isActive = (path) => location.pathname.startsWith(path);

  const navItems = [
    { path: '/', icon: LayoutDashboard, label: 'Dashboard' },
    { path: '/jobs', icon: Briefcase, label: 'All Jobs' },
    { path: '/applied-jobs', icon: FileText, label: 'Applied Jobs' },
      { path: '/interview-schedules', icon: User, label: 'Interview Schedules' }, 
